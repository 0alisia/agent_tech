...
print("Names imported from Addmodules:", [name for name in dir() if 'C3k2' in name])
import contextlib
import pickle
import re
import types
...
from ultralytics.nn.modules.iCAFM import C_iCAFM
...
# 在ultralytics/nn/tasks.py中添加导入
try:
    from .modules.iCAFM import C_iCAFM_v2
    C_iCAFM = C_iCAFM_v2  # 别名映射
    print("✓ Successfully imported C_iCAFM module")
except ImportError as e:
    print(f"✗ Failed to import C_iCAFM: {e}")
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect,AFPN4Head)):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            s = 256  # 2x min stride
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect,AFPN4Head}
        ):
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, YOLOEDetect, v10Detect,AFPN4Head)):
                return "detect"
